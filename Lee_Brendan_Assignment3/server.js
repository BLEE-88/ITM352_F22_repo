...
app.post('/add_cart', function (request, response) {
    ...
    for (i in quantities) { 
        ...
        //Attempt at putting products key in shopping cart
        if (!valid_quantities) {
            var name = "products";
            let POST = request.body;
            let prodname = POST["products"];
            request.session.cart[products_key] = shopping_cart;

            if (shopping_cart[products_key] == undefined); { //If we do not have a request.session.cart make one
                shopping_cart[products_key] = {};
                shopping_cart[products_key].name = prodname;
                shopping_cart[products_key].quantity = quantity;
                shopping_cart[products_key].price = price;
            }
        } else {
    ...
    app.post("/login", function (request, response) {
        ...
        if (typeof users[user_name] != 'undefined') {
            if (users[user_name].password == the_password) {
                ...
                response.cookie('username', username, { maxAge: 30 * 60 * 1000 });
            } else {
